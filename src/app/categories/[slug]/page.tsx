...
export default async function CategoryCatalogPage({ params, searchParams }: CategoryPageProps) {
  ...
  const currentCategory = await prisma.category.findFirst({
    where: { slug: currentCategorySlug, isActive: true }
  });
  ...
  // 2. Fetch active categories for the sidebar checklist
  ...
  // 5. Apply sorting logic at JavaScript level (Decimal parsing safety)
  products.sort((a, b) => {
  ...
  // 7. Helper function to generate modified filter URLs for Server-side routing
  const getFilterUrl = (params: {
    ...
    const targetSlug = params.categorySlug !== undefined ? params.categorySlug : currentCategorySlug;
    if (targetSlug === null) {
      // If we clicked "All Products", we navigate to /products
      return `/products?${query.toString()}`;
    }

    return `/categories/${targetSlug}?${query.toString()}`;
  };
  ...
  return (
    <main className="min-h-screen bg-[#FAF9F6] text-gray-900 pb-24">
      <PageHeader 
        ...
        description={`Bộ sưu tập các mẫu mã mới nhất thuộc danh mục ${currentCategory.name} cao cấp.`}
          ...
          <aside className="hidden lg:block space-y-8 bg-white p-6 rounded-brand-lg border border-gray-100/60 shadow-xs">
            <div className="flex items-center justify-between pb-4 border-b border-gray-100">
              ...
              {(searchQuery || minPrice !== undefined || maxPrice !== undefined) && (
                <Link
                  href={`/categories/${currentCategorySlug}`}
            ...
            <div className="space-y-3">
              ...
              <form action={`/categories/${currentCategorySlug}`} method="GET" className="relative">
            ...
            <div className="space-y-3 pt-4 border-t border-gray-100">
              ...
              <div className="space-y-2 flex flex-col">
                <Link
                  href={getFilterUrl({ categorySlug: null })}
                  className="text-xs font-bold transition-all py-1.5 px-2.5 rounded-brand-sm border border-transparent text-gray-500 hover:text-gray-900 hover:bg-gray-50"
                >
                  Tất cả sản phẩm
                </Link>
                {categories.map((cat) => {
                  ...
                  return (
                    <Link
                      key={cat.id}
                      href={getFilterUrl({ categorySlug: cat.slug })}
            ...
            <div className="space-y-3 pt-4 border-t border-gray-100">
              ...
              <form action={`/categories/${currentCategorySlug}`} method="GET" className="space-y-3">
          ...
          <div className="lg:col-span-3 space-y-6">
            ...
            {paginatedProducts.length === 0 && (
              <EmptyState
                ...
                actionHref={`/categories/${currentCategorySlug}`}
